...
tblink_rpc_hdl_dir = os.path.dirname(os.path.abspath(__file__))

if os.path.isdir(os.path.join(tblink_rpc_hdl_dir, "packages")):
    print("Packages are inside this directory")
    packages_dir = os.path.join(tblink_rpc_hdl_dir, "packages")
else:
    parent = os.path.dirname(tblink_rpc_hdl_dir)
...
#if os.path.isdir(os.path.join(cwd, "build")):
...
setup(
  ...
  entry_points={
    'console_scripts': [
      'tblink-rpc-hdl = tblink_rpc_hdl.__main__:main'
    ]
  },
  setup_requires=[
    'setuptools_scm',
  ...
  ext_modules=[
        Extension("tblink_rpc_hdl.tblink_rpc_hdl_stub",
            extra_compile_args=extra_compile_args,
#            include_dirs=[
#                os.path.join(pybfms_root, 'ext/common'), 
#                os.path.join(pybfms_root, 'ext/hdl_sim')],
            sources=[
                os.path.join(tblink_rpc_hdl_dir, 'ext', "tblink_rpc_hdl_stub.pyx"), 
            ],
#            libraries=[
#                "-Wl,--whole-archive",
#                "tblink_rpc_hdl",
